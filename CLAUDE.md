...
**IA da plataforma com fila automática** (Edge Function `assistente-ia`, repositório da Biblioteca; motor em `supabase/functions/_comum/ia.ts`): Gemini → Groq → Cerebras e Mistral → gratuitos do OpenRouter (11 modelos `:free`, conferidos em openrouter.ai/api/v1/models em 24/09/2026) → GitHub Models, SambaNova, NVIDIA, Cohere, Hugging Face → restante das filas antigas → pagos. Provedor sem chave é pulado. Modelo que responde 429 **descansa** (10 min; 6 h se o limite é diário), 401/403 tira o provedor por 1 h, 404 tira o modelo por 1 dia — a fila segue sozinha. Teto de 110 s para a fila inteira. PDF só vai para Gemini ou OpenRouter (tipo `file`). `{provedores:true}` devolve quais estão ligados (a aba I.A lista as IAs com "ligada / sem chave no servidor").
- **Chaves (supabase secrets set, pasta da Biblioteca):** hoje só `GEMINI_API_KEY`, `GROQ_API_KEY`, `OPENROUTER_API_KEY`. Para ligar os outros: `CEREBRAS_API_KEY`, `MISTRAL_API_KEY`, `GITHUB_MODELS_TOKEN`, `NVIDIA_API_KEY`, `SAMBANOVA_API_KEY`, `COHERE_API_KEY`, `HF_TOKEN`. **Camada paga** (DeepSeek V4 Flash, Gemini 2.5 Flash e Llama 3.3 70B pelo OpenRouter `:floor`, e `DEEPSEEK_API_KEY`) só com `IA_PAGOS=1` e sempre depois de todas as gratuitas.
...
- **Teste de ponta a ponta (24/09/2026):** conta temporária semeada com uma escola e uma turma, Meu Diário local no Chrome sem janela: migração, pastas, 26 ferramentas, prompt preenchido, resposta real da IA da plataforma (a fila caiu para Groq GPT-OSS 120B sozinha), identificação de turma/disciplina/bimestre, pasta automática, gravação no banco e os 6 formatos + ZIP. Conta apagada no fim.
