...
**E-mail verificado depois (Etapa 15, 22/09/2026, decisão do professor):** "Confirm email" DESLIGADO no Supabase (`mailer_autoconfirm = true`, mudado pela API de gestão). Com isso o Supabase preenche `email_confirmed_at` em todo cadastro, e essa coluna NÃO diz mais se o e-mail foi verificado. A verificação fica em `public.conta_email_verificado` (só leitura do dono), gravada apenas por `conta_email_status()`: Google com `email_verified` = verificado; sessão aberta por link/código de e-mail (`amr` com `otp`/`magiclink`) = verificado; trocou o e-mail = volta a pendente. Contas anteriores a 22/09 entraram como verificadas. No `perfil.html`, card 🚪 Conta: selo "⚠ Verificar e-mail" / "✓ E-mail verificado" (também no topo) e o botão "Verificar e-mail", que manda o modelo "Link mágico" (link + código de 8 dígitos) por `signInWithOtp({shouldCreateUser:false})`; o código é digitado no próprio perfil (`verifyOtp`), então funciona com o e-mail aberto no celular. API: `S.auth.emailStatus()`, `enviarVerificacaoEmail()`, `confirmarCodigoEmail(codigo)`. SQL em `supabase/2026-09-22-etapa15-email-verificado.sql`.
