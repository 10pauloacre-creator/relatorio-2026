...
**Gravidade das ocorrências (Etapa 4):** a Edge Function `classificar-ocorrencias` fica no repositório da Biblioteca (`supabase/functions`) e usa a fila de modelos de `_comum/ia.ts`; o deploy é feito com `supabase functions deploy classificar-ocorrencias --use-api`. Ela sugere leve/médio/grave/muito grave com justificativa (`relatorio_registrar_sugestao_gravidade`) e nunca sobrescreve gravidade confirmada. `assets/js/relatorio-ocorrencias.js` mostra um selo por aluno em cada `.oi` (`data-runtime-ui`) e chama a IA depois de publicar; o professor confirma ou troca com `relatorio_definir_gravidade`. Ocorrências positivas não são classificadas. **Comportamento não desconta nota.**
