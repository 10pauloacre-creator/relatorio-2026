...
**Vídeo de marketing (25/09/2026):** 60 s, 1920×1080, com trilha original e efeitos sonoros, contando problema → solução → "Crie sua conta grátis". Fonte em `marketing/video/`: `cena.html` (cenas em HTML/CSS/JS + fundo em GLSL; `window.renderAt(t)` desenha qualquer instante), `render.js` (Node + Puppeteer tira os quadros e o ffmpeg codifica), `trilha.py` (Python + numpy/scipy sintetiza música e efeitos nos instantes que as cenas declaram em `sons`) e `gerar-video.ps1` (faz tudo). O mestre (`marketing/RELATORIO-SKIN-video-marketing.mp4`, ~44 MB) fica só na máquina (.gitignore). A versão do site é `assets/video/relatorio-skin-60s.mp4` (720p, ~5 MB) + `relatorio-skin-capa.jpg` (quadro de 18,66 s); `node render.js --so-site` refaz as duas a partir do mestre. **No site**, o vídeo fica no hero do `index.html`, no lugar do antigo visual à direita: cartão 16:9; no computador, prévia sem som em loop quando aparece na tela (nunca com "reduzir movimento", economia de dados ou 2G; no celular só a capa com ▶ e a legenda "Toque para assistir"); o clique abre o `<dialog id="vmodal">` com o vídeo com som, e no fim aparecem "Criar conta grátis" e "Praticar na demonstração". Detalhes e roteiro em `marketing/video/README.md`. Ao mudar o vídeo, refaça a versão do site e suba `assets/video/`.
