-- ═══════════════════════════════════════════════════════════════════════════
-- ETAPA 6 — Bônus do Ranking de Poder na média final
-- ───────────────────────────────────────────────────────────────────────────
-- A tela do ranking (Biblioteca) promete "+N na média" por nível. Decisão do
-- professor (17/09/2026): as médias são anuais, então o +N vale sobre a soma
-- dos 4 bimestres, ou seja, +N÷4 na MÉDIA FINAL do ano (Mago Supremo: +8 → +2),
-- sem mudar as notas dos bimestres nem a recuperação. Limite de 10.
--
--   Novato/Aprendiz +0 · Camponês +0,25 · Gladiador +0,5 · Rei +1,5 · Mago +2
--
-- O cálculo fica no motor único (assets/js/boletim-regras.js, nivelPoder).
-- Aqui: interruptor por escola, pontos de cada aluno do painel e o boletim
-- do aluno. Hermínio: desligado (boletins mantidos como estão).
-- Idempotente.
-- ═══════════════════════════════════════════════════════════════════════════

ALTER TABLE public.relatorio_regras_escola
  ADD COLUMN IF NOT EXISTS bonus_poder boolean NOT NULL DEFAULT false;

UPDATE public.relatorio_regras_escola r SET bonus_poder = true, atualizada_em = now()
  FROM public.escolas e
 WHERE e.id = r.escola_id AND e.slug = 'padre-carlos-casavequia' AND NOT r.bonus_poder;

-- Pontos e nível de cada aluno do painel (só o professor).
CREATE OR REPLACE VIEW public.relatorio_poder_alunos AS
SELECT v.scope_key, v.aluno_relatorio_id, v.nome_relatorio, v.aluno_id,
       COALESCE(a.pontos, 0) AS pontos, COALESCE(a.nivel, 0) AS nivel,
       COALESCE(rg.bonus_poder, false) AS bonus_poder
FROM public.relatorio_aluno_vinculo v
JOIN public.alunos a ON a.id = v.aluno_id
JOIN public.relatorio_turmas t ON t.scope_key = v.scope_key
LEFT JOIN public.relatorio_regras_escola rg ON rg.escola_id = t.escola_id
WHERE (SELECT private.is_relatorio_admin());

REVOKE ALL ON public.relatorio_poder_alunos FROM anon;
GRANT SELECT ON public.relatorio_poder_alunos TO authenticated;

-- Boletim do aluno: regra da escola e pontos do ranking.
  ...
  LOOP
    ...
    v_resultado := v_resultado || jsonb_build_array(jsonb_build_object(
      ...
      'regras', jsonb_build_object('bimestresAutomaticos', to_jsonb(v_regras.automaticos), 'recuperacaoSemestral', v_regras.recuperacao,
                                   'bonusPoder', v_regras.bonus_poder),
