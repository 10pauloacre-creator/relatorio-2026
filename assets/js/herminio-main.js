...
// Alunos cadastrados na Biblioteca e ausentes da lista acima (Etapa 5B):
// entram no fim, com a data de entrada. Aulas anteriores não contam para eles.
if (window.RelatorioAlunosAdicionados) {
  RelatorioAlunosAdicionados.completarDiario('raimundo-herminio-de-melo-2', ALUNOS_RH);
  RelatorioAlunosAdicionados.atualizar('raimundo-herminio-de-melo-2');
}
function rhAntesDaEntrada(turmaId, numeroAluno, paneId) {
  if (!window.RelatorioAlunosAdicionados) return false;
  var aluno = (ALUNOS_RH[turmaId] || []).filter(function (a) { return a.n === Number(numeroAluno); })[0];
  return !!aluno && RelatorioAlunosAdicionados.foraDaAula(aluno, rhDiaBasePane(paneId));
}
...
if (textoTitulo.indexOf('+') >= 0) return;
var paneResumo = card.querySelector('.ipane[id^="r-all-"], .ipane[id^="p-all-"], .ipane[id^="a-all-"]');
if (!paneResumo) return;
var refResumo = paneResumo.id.match(/^[rpa]-all-\d{4}(t89|t1|t23)(lp|ing|esp|art|red)$/);
if (!refResumo) return;
...
var data = parseDataCardRH(card);
if (!data) return;
if (ultimaDataTurma[turmaId] && data <= ultimaDataTurma[turmaId]) return;
var dataKey = rhDataChaveIso(data);
...
if (rhAntesDaEntrada(rhDescobrirTurmaPane(pane), numeroAluno, pane.id)) return null; 
...
function rhMontarRetratoLancamentos() {
  ...
  Object.keys(ALUNOS_RH).forEach(function (turmaId) {
    retrato.turmas.push({
      codigo: turmaId,
      alunos: (ALUNOS_RH[turmaId] || []).map(function (a) { return [a.n, a.nm, !!a.tr, !!a.bib]; })
    });
  });
